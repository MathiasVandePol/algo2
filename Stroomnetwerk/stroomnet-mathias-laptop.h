...
template <class T>
Vergrotendpadzoeker<T>::Vergrotendpadzoeker(const GraafMetTakdata<GERICHT ,T>& _q,int _v,int _v2,Pad<T>& _p):p(_p), q(_q),v(_v),v2(_v2),l(q.aantalKnopen()), m(q.aantalKnopen(),false){
	p.clear();
	assert(v != v2);
	foo(v,0);
	assert(p.size()!=1);
...
class Stroomnetwerk:public GraafMetTakdata<GERICHT, T >{
	public:
		...
		friend ostream& operator-=(const Pad& p){
			for (int i = 1; i < p.size(); i++){
				T van = p[i - 1];
				T naar = p[i];
				TakData* verbinding = this->geefTakData(van, naar);
				*verbinding -= p.geefCapaciteit;
